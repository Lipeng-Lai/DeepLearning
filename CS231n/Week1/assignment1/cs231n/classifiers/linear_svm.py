...
def svm_loss_naive(W, X, y, reg):
    ...
    num_classes = W.shape[1] # C col
    num_train = X.shape[0] # N row
    loss = 0.0
    
    for i in range(num_train):
        scores = X[i].dot(W)
        correct_class_score = scores[y[i]] # the score of the correct label
        for j in range(num_classes): 
            if j == y[i]: 
                continue # skip the same label
            # SVM Algorithm
            margin = scores[j] - correct_class_score + 1 # note delta = 1
            if margin > 0: # max(0, margin)
                loss += margin # update the weight array
                dW[:, j] += X[i]*1.0 # if loss, the else label += value  
                dW[:, y[i]] -= X[i]*1.0 # if loss, the correct lable -= value
